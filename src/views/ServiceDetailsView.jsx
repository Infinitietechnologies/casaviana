...
const ServiceDetailsView = () => {
  ...
  useEffect(() => {
    if (
      videoRef.current &&
      activeSlideIndex === imagesToDisplay.length &&
      service?.video?.type === "upload"
    ) {
      videoRef.current.play();
    }
  }, [activeSlideIndex, service, imagesToDisplay]);
  ...
  const handleSlideChange = (swiper) => {
    ...
    if (videoRef.current && service?.video?.type === "upload") {
      const totalSlides = imagesToDisplay.length;
      const videoSlideIndex = service?.video ? totalSlides : -1;
                        ...
                        <SwiperSlide key="video-slide">
                          <div className="relative w-full h-full">
                            {service.video.type === "upload" ? (
                              ...
                              <iframe
                                src={getEmbedUrl(service.video.url)}
